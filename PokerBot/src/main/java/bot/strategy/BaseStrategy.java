 ...
 */
public abstract class BaseStrategy implements IBotStrategy{
    /**
     ...
     * @return возвращает количество аутов для сбора стрита
     */
    protected int findDrawStraight (Card[] hand, Card[] table){
        List<Card> cards = new ArrayList<Card>();
        cards.add(hand[0]);
        cards.add(hand[1]);
        for(int i = 0; i < table.length; i++)
